...
const Logo: React.FC<LogoProps> = ({
  ...
  return (
    <Link
      ...
      aria-label={user ? 'Hablemos Cripto, ir a tus lecciones' : 'Hablemos Cripto, ir al inicio'}
    >
      <span
        className={`bg-gradient-to-br from-brand-400 to-brand-600 ${chip} shadow-glow-brand flex items-center justify-center group-hover:rotate-6 transition-transform duration-500`}
      >
        <Bitcoin className={`text-navy-950 ${glyph}`} aria-hidden="true" />
      </span>
      <span
        className={`font-heading font-extrabold text-white tracking-tighter ${word} ${wordmarkClassName}`}
      >
