  ...
  useEffect(() => {
    const evaluateSelection = () => {
      ...
      if (text.length > 2 && text.length < 150) {
        ...
        const top = coarse ? rect.bottom + 12 : rect.top - 10;
        setSelection({
          visible: true,
          x: clampedX,
          y: !coarse && top < 48 ? rect.bottom + 44 : top,
          text,
        });
      } else {
        setSelection((prev) => (prev.visible ? { ...prev, visible: false } : prev));
  ...
  return (
    ...
    >
      <button
        ...
        className="flex min-h-11 items-center gap-2 px-4 py-2.5 bg-brand-500 hover:bg-brand-400 text-navy-950 text-xs font-bold rounded-full shadow-glow-brand transition-colors whitespace-nowrap"
      >
