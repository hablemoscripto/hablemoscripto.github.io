...
export default function SectionRenderer({
  ...
  return (
    <div>
      <div className="mb-8">
        {section.title && section.type !== 'takeaways' && (
          <h2 id={headingId} className="text-2xl font-bold text-white mb-4 scroll-mt-28">
            {section.title}
          </h2>
        )}
        {onAskSection &&
        ...
        {section.image && (
          <div className="my-6 not-prose">
            <button
              ...
              onClick={() =>
                onImageClick(section.image!, section.imageAlt || section.title || 'Infographic')
              }
            >
              <img
                src={section.image}
              ...
              />
              <span className="absolute bottom-3 right-3 inline-flex items-center gap-1.5 rounded-lg bg-navy-950/85 px-3 py-1.5 text-xs font-medium text-white pointer-events-none">
                <ZoomIn size={14} aria-hidden="true" />
                Ampliar
              </span>
            </button>
            {section.imageCaption && (
              <p className="text-sm text-navy-400 text-center mt-2 italic">
                {section.imageCaption}
              </p>
            )}
        ...
        {infographicNode && <div className="my-6 not-prose">{infographicNode}</div>}

        {/* Features Grid */}
        {!hasInfographic && section.features && section.features.length > 0 && (
          <div className="grid gap-4 my-6 not-prose">
            {section.features.map(
              (feature: { icon?: unknown; title?: string; text?: string }, i: number) => {
                ...
                return (
                  <div
                    key={i}
                    className="bg-navy-900/50 p-5 rounded-xl border border-navy-700/50 hover:border-brand-500/30 transition-colors"
                  >
        ...
        {!hasInfographic && section.highlight && (
          <div className="bg-brand-500/10 border border-brand-500/30 rounded-xl p-5 my-6 not-prose">
            <h3 className="font-bold text-brand-400 mb-2">{section.highlight.title}</h3>
          ...
          <div className="relative grid md:grid-cols-2 gap-4 my-6 not-prose">
            ...
            <div className="bg-gradient-to-br from-green-500/10 to-green-500/5 p-6 rounded-xl border border-green-500/30 relative overflow-hidden">
              ...
              {section.leftSide?.points ? (
                ...
                <p className="text-sm text-navy-400">
                  Descentralizado, deflacionario, instantáneo, barato.
                </p>
              )}
            </div>
            ...
            <div className="bg-gradient-to-br from-red-500/10 to-red-500/5 p-6 rounded-xl border border-red-500/30 relative overflow-hidden">
              ...
              {section.rightSide?.points ? (
                ...
                <p className="text-sm text-navy-400">
                  Centralizado, inflacionario, lento, costoso.
                </p>
              )}
            </div>
          </div>
                ...
                <div key={i} className="pl-2 border-l-2 border-brand-500/30">
                  ...
                  {t.whyItMatters && (
                    <dd className="text-xs text-navy-400 mt-2 italic">
                      <span className="text-brand-400/80 font-semibold not-italic">
                        Por qué importa:{' '}
                      </span>
                      {t.whyItMatters}
                    </dd>
        ...
        {section.type === 'takeaways' && (
          <div className="bg-brand-500/10 border border-brand-500/20 rounded-xl p-6 my-8 not-prose">
            <h2
              id={headingId}
              className="flex items-center gap-2 text-xl font-bold text-brand-400 mb-4 scroll-mt-28"
            >
