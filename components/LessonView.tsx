...
const LessonView: React.FC = () => {
  ...
  const { prevLesson, nextLesson, canGoNext } = useLessonNavigation(
    id,
      ...
      <UpgradePaywall
        ...
        completedContext={
          graduated
            ? {
                completedLevelTitle:
                  lessonLevel === 'advanced' ? LEVEL_TITLES.intermediate : LEVEL_TITLES.beginner,
      ...
      <div className="min-h-screen flex items-center justify-center text-white bg-navy-950 p-4">
        <div className="text-center max-w-md bg-navy-900/50 p-8 rounded-2xl border border-white/10 backdrop-blur-sm">
          ...
          <p className="text-navy-400 mb-6 leading-relaxed">
            Puede ser un problema temporal de conexión. Tu acceso está intacto. Intenta de nuevo en
            un momento.
          </p>
          <div className="flex flex-col gap-3">
            ...
            <a
              href={`mailto:soporte@hablemoscripto.io?subject=${encodeURIComponent(`No carga la lección ${id}`)}`}
            ...
            {!isFocusMode && lesson.referrals && lesson.referrals.length > 0 && (
              <div className="grid gap-4 my-8">
                {lesson.referrals.map((ref, idx) => (
                  <div
                    key={idx}
                    className="bg-gradient-to-r from-navy-900 to-navy-800 border border-brand-500/30 rounded-xl p-6 relative overflow-hidden group hover:border-brand-500/50 transition-all"
                  >
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="px-2 py-1 bg-brand-500/20 text-brand-400 text-xs font-bold rounded uppercase tracking-wider">
                            Recomendado
                          </span>
              ...
              {!showQuiz ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-navy-400 text-sm mt-2 mb-6">
                    Necesitas acertar al menos el 70% de las preguntas para completar la lección.
                ...
                <>
                  ...
                  {quizPassed && levelJustCompleted && (
                    <div
                      className="mt-8 text-center animate-in zoom-in"
                      role="status"
                      aria-live="polite"
                    >
                      <div className="inline-flex flex-col items-center gap-4 p-8 rounded-2xl bg-gradient-to-br from-brand-500/15 to-brand-400/5 border border-brand-500/30">
                        ...
                        <p className="text-navy-200 text-sm max-w-sm">
                          Terminaste todas las lecciones de este nivel. Reclama tu certificado y
                          compártelo.
                        </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {prevLesson ? (
                  ...
                  >
                    <div className="shrink-0 w-10 h-10 bg-navy-800 group-hover:bg-navy-700 rounded-lg flex items-center justify-center transition-colors">
                      <ArrowLeft
                        size={20}
                        className="text-navy-400 group-hover:text-white transition-colors"
                      />
                    </div>
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-navy-300 group-hover:text-white truncate transition-colors">
                        {prevLesson.title}
                      </p>
                    </div>
                  </button>
                ...
                {nextLesson && (
                  ...
                  >
                    <div className="min-w-0">
                      <p
                        className={`text-xs uppercase tracking-wider mb-1 ${canGoNext ? 'text-brand-400' : 'text-navy-400'}`}
                      >
                    ...
                    <div
                      className={`shrink-0 w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${
                        canGoNext ? 'bg-brand-500/20 group-hover:bg-brand-500/30' : 'bg-navy-800/50'
                      }`}
                    >
                      {canGoNext ? (
                        <ArrowRight
                  ...
                  {hasCommunityAccess(entitlements) && (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-navy-300 group-hover:text-white text-sm transition-colors">
                          Comunidad
                        </span>
                      </div>
                      <ExternalLink
                        size={14}
                        className="text-navy-400 group-hover:text-brand-400 transition-colors"
                      />
                    </a>
                  )}
                  <a
                    href={`mailto:soporte@hablemoscripto.io?subject=${encodeURIComponent(`Feedback · Lección ${lesson.number?.split(' ')[0] || id}: ${lesson.title}`)}`}
                    className="w-full flex items-center justify-between p-3 bg-navy-800 rounded-xl hover:bg-navy-700 hover:border-brand-500/30 border border-transparent transition-all text-left group"
                  >
                    <div className="flex items-center gap-3">
