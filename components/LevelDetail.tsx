...
const LevelDetail: React.FC<LevelDetailProps> = ({ levelData }) => {
    ...
    return (
        <div className="min-h-screen bg-surface-base pb-20">
            {/* Header */}
            <div className="bg-surface-1 border-b border-surface-border pt-24 pb-12">
                <div className="container max-w-5xl mx-auto px-6">
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        ...
                        <div className="bg-surface-2 p-4 rounded-xl border border-surface-border-hover min-w-[200px]">
                    ...
                    {levelData.modules.map((module, moduleIdx) => (
                        <div key={module.id} className="animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${moduleIdx * 100}ms` }}>
                            <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-3">
                                <span className="w-8 h-8 rounded-lg bg-surface-3 flex items-center justify-center text-sm text-slate-400 border border-surface-border-hover">
                                    {moduleIdx + 1}
                                </span>
                                {module.title}
                            </h3>
                                ...
                                {module.lessons.map((lesson, lessonIdx) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            disabled={isLocked}
                                            className={`group flex items-center gap-4 p-4 bg-surface-2 border border-surface-border rounded-xl transition-all text-left ${
                                                isLocked
                                                    ? 'opacity-50 cursor-not-allowed'
                                                    : 'hover:bg-surface-1 hover:border-brand-500/30'
                                            }`}
                                        >
                                            <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 transition-colors ${
                                                isCompleted
                                                    ...
                                                    : 'bg-surface-3 text-slate-400 group-hover:bg-brand-500/10 group-hover:text-brand-500'
                                                }`}>
