...
import { ChevronLeft, Lock, CheckCircle, PlayCircle, Clock, Award } from 'lucide-react';
...
const LevelDetail: React.FC<LevelDetailProps> = ({ levelData }) => {
  ...
  if (!entitlementsLoading && !canAccessLevel(entitlements, levelData.id)) {
    ...
    const prevLevelIds =
      levelData.id === 'intermediate'
        ? getBeginnerLessonIds()
        : levelData.id === 'advanced'
          ? getIntermediateLessonIds()
          : [];
    const graduated =
  ...
  const globalProgress = allLessons.length
    ? Math.round((globalCompleted / allLessons.length) * 100)
    : 0;
    ...
    <>
      ...
      {showCertificate && (
        <Certificate
          ...
          variant={levelData.id as CourseLevelId}
          ...
          <div className="container max-w-5xl mx-auto px-6">
            ...
            >
              ...
              <span className="text-brand-500">
                Nivel {LEVEL_LABELS[levelData.id] ?? levelData.id}
              </span>
            </nav>

            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div>
                <span className="text-brand-500 font-bold tracking-wider uppercase text-sm mb-2 block">
                  Nivel {LEVEL_LABELS[levelData.id] ?? levelData.id}
                </span>
              ...
              <div className="bg-navy-800/50 p-5 rounded-2xl border border-navy-700 min-w-[220px]">
                ...
                {levelComplete && (
                  <button
                    ...
                    className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-bold text-navy-950 transition-colors hover:bg-brand-400"
                  >
                    ...
                    {module.lessons.map((lesson, lessonIdx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div
                            className={`transition-opacity ${isLocked ? 'opacity-0' : 'opacity-0 group-hover:opacity-100'} text-brand-500`}
                          >
                            <PlayCircle size={24} />
                          </div>
                        </button>
