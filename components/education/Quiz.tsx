    ...
    <div className="space-y-3">
      <div role="radiogroup" aria-label="Opciones de respuesta" className="space-y-3">
        {q.options.map((opt, optIdx) => {
          let optionClass =
            'w-full p-4 rounded-xl text-left border transition-all flex items-center gap-3 group ';
          ...
          if (submitted) {
            ...
            if (answer === optIdx) {
              optionClass +=
                'bg-brand-500/10 border-brand-500 text-white shadow-[0_0_15px_rgba(245,158,11,0.15)]';
            } else {
              optionClass +=
                'bg-navy-950 border-navy-800 text-navy-300 hover:border-navy-600 hover:bg-navy-900 cursor-pointer';
            }
          }
          ...
          return (
            ...
            >
              ...
              {submitted && answer === optIdx && optIdx !== q.correctAnswer && (
        ...
        {options.map(({ value, label, icon }, optIdx) => {
          ...
          if (submitted) {
            ...
            if (answer === value) {
              ...
              optionClass +=
                'bg-navy-950 border-navy-800 text-navy-300 hover:border-navy-600 cursor-pointer';
            }
          }
          ...
          return (
            <button
              ...
              tabIndex={submitted ? 0 : isTabStop ? 0 : -1}
              onKeyDown={(e) =>
                moveRadioFocus(e, optIdx, options.length, (i) => onAnswer(options[i].value))
              }
...
const MultipleSelectRenderer: React.FC<QuestionComponentProps> = ({
  ...
  return (
    <div className="space-y-3">
      <div
        role="group"
        aria-label="Selecciona todas las respuestas correctas"
        className="space-y-3"
      >
        {q.options.map((opt, optIdx) => {
...
const Quiz: React.FC<QuizProps> = ({
  ...
  const persistPass = async () => {
    ...
    const result = await Promise.resolve(onComplete(correctCount));
    ...
    if (unanswered.length > 0) {
      setSubmitError(
        `Por favor responde todas las preguntas. ${
          unanswered.length === 1 ? 'Falta 1 pregunta' : `Faltan ${unanswered.length} preguntas`
        }.`
      );
        ...
        {isRevealed && (
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                <p
                  className={`font-medium mb-1 ${questionCorrect ? 'text-emerald-400' : 'text-amber-400'}`}
                >
      ...
      <div className="p-8">
        {viewMode === 'single' && !submitted ? (
          <div className="space-y-8">
            ...
            <p className="text-xs text-navy-400 text-center">
              Necesitas el 70% para completar la lección · Sin límite de tiempo
            </p>
            ...
            {answeredCount === questions.length && currentQuestionIndex < questions.length - 1 && (
              ...
              >
                Enviar Quiz ({questions.length}/{questions.length} respondidas){' '}
                <ArrowRight size={18} />
              </button>
            )}
          </div>
        ) : (
          <div className="space-y-10">{questions.map((q, idx) => renderQuestion(q, idx))}</div>
        )}
        ...
        {submitted && (
          <div
            role="status"
            aria-live="polite"
            className={`mt-10 p-8 rounded-2xl text-center border transition-all duration-500 ${
              passed && persistStatus === 'saved'
                ? 'bg-emerald-500/10 border-emerald-500/30 shadow-[0_0_40px_rgba(16,185,129,0.1)]'
                : passed && persistStatus === 'error'
          ...
          >
            <div className="flex justify-center mb-5">
              {passed && persistStatus === 'saved' ? (
                <div className="relative">
                  ...
                  <div className="absolute -top-2 -right-2 px-3 py-1 bg-brand-500 text-navy-900 rounded-full text-xs font-black uppercase tracking-wider">
                    +100 XP
                  </div>
                </div>
              ) : passed && persistStatus === 'saving' ? (
                <div className="w-24 h-24 rounded-3xl bg-navy-700 flex items-center justify-center">
                  <Loader2 size={44} className="text-brand-400 animate-spin" aria-hidden="true" />
                </div>
              ) : (
                <div className="w-24 h-24 rounded-3xl bg-navy-700 flex items-center justify-center">
            ...
            <p
              className={`text-3xl font-bold mb-3 ${passed && persistStatus === 'saved' ? 'text-emerald-400' : 'text-brand-500'}`}
            >
              {correctCount} / {questions.length}
            </p>

            <p className="text-navy-400 mb-6">{percentage}% de respuestas correctas</p>

            {passed && persistStatus === 'saved' ? (
              <p className="text-emerald-400/80 text-sm">
                Excelente trabajo. Demostraste dominio del tema.
              </p>
            ) : passed && persistStatus === 'saving' ? (
              <p className="text-navy-300 text-sm">Un momento. Estamos registrando tu resultado.</p>
            ) : passed && persistStatus === 'error' ? (
              <div className="space-y-4">
                ...
                <button
                  onClick={() => void persistPass()}
                  className="px-6 py-3 bg-brand-500 hover:bg-brand-400 text-navy-900 font-bold rounded-xl transition-all inline-flex items-center gap-2"
                >
                  <RotateCcw size={18} aria-hidden="true" /> Reintentar guardado
                </button>
              </div>
            ) : (
              <div className="space-y-4">
                <p className="text-navy-300">
                  Necesitas el 70% para aprobar, te{' '}
                  {passThreshold - correctCount === 1 ? 'falta' : 'faltan'}{' '}
                  <span className="text-white font-bold">{passThreshold - correctCount}</span>{' '}
                  {passThreshold - correctCount === 1
                    ? 'respuesta correcta más'
                    : 'respuestas correctas más'}
                  .
                </p>
                <p className="text-sm text-navy-400">
