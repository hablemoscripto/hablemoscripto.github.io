...
type CheckpointDraft = {
  ...
  showResult: { [key: string]: boolean };
  currentQuestionIndex: number;
  completed: boolean;
};

function loadCheckpointDraft(id: string): CheckpointDraft {
  const empty: CheckpointDraft = {
    answers: {},
    showResult: {},
  ...
  try {
    const raw = window.sessionStorage.getItem(`hc_checkpoint_${id}`);
    if (!raw) return empty;
    const parsed = JSON.parse(raw) as Partial<CheckpointDraft>;
    return {
...
const CheckpointQuiz: React.FC<CheckpointQuizProps> = ({
  ...
  const [draft] = useState(() => loadCheckpointDraft(id));
  const [expanded, setExpanded] = useState(defaultExpanded);
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(draft.currentQuestionIndex);
  const [answers, setAnswers] = useState<{ [key: string]: number }>(draft.answers);
  const [showResult, setShowResult] = useState<{ [key: string]: boolean }>(draft.showResult);
  ...
  useEffect(() => {
    ...
    try {
      window.sessionStorage.setItem(
        `hc_checkpoint_${id}`,
        JSON.stringify({ answers, showResult, currentQuestionIndex, completed })
      );
    } catch {
      /* */
    }
  }, [id, answers, showResult, currentQuestionIndex, completed]);
  ...
  const resetCheckpoint = () => {
    ...
    setCompleted(false);
    try {
      window.sessionStorage.removeItem(`hc_checkpoint_${id}`);
    } catch {
      /* */
    }
  };
          ...
          {completed && (
            <span
              className={`text-xs font-medium px-2 py-1 rounded ${
                allCorrect ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'
              }`}
            >
                ...
                {currentQuestion.options.map((opt, optIdx) => {
                  ...
                  return (
                    <button
                      ...
                      onKeyDown={(e) =>
                        !showFeedback &&
                        moveRadioFocus(e, optIdx, currentQuestion.options.length, (i) =>
              ...
              {currentQuestion.hint && !showResult[currentQuestion.id] && (
                <>
                  <button
                    ...
                    className="flex items-center gap-1.5 min-h-11 px-1 text-xs text-navy-400 hover:text-brand-400 transition-colors"
                  >
                    <Lightbulb size={14} />
